...
def bottleneck(inputs, depth, depth_bottleneck, stride, rate=1,
    ...
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        if depth == depth_in:
            shortcut = resnet_utils_tf1.subsample(inputs, stride, 'shortcut')
        else:
        ...
        residual = resnet_utils_tf1.conv2d_same(residual, depth_bottleneck, 3, stride, rate=rate, scope='conv2')
...
def resnet_v2(inputs,
    ...
    with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, bottleneck, resnet_utils_tf1.stack_blocks_dense],
            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                if include_root_block:
                    ...
                    with slim.arg_scope([slim.conv2d], activation_fn=None, normalizer_fn=None):
                        net = resnet_utils_tf1.conv2d_same(net, 64, 7, stride=2, scope='conv1')
                    net = slim.max_pool2d(net, [3, 3], stride=2, scope='pool1')
                net = resnet_utils_tf1.stack_blocks_dense(net, blocks, output_stride)
